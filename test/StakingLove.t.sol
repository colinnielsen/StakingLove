...
contract StakingLoveTest is DSTest {
    ...
    function testStake() public {
        ...
        uint256 stakingTime = stakinglove.userStakingmiloscStakingInfosTime(
            address(nft),
            1
        );
    ...
    function testUnstake() public {
        ...
        uint256 stakeTime = stakinglove.miloscStakingInfosTime(
            address(nft),
            tokenId
        );
        assertEq(
            stakeTime,
            0,
            "miloscStakingInfosTime mapping not reset correctly"
        );
    }
    ...
    function testUnstakeBunch2() public {
        ...
        for (uint256 i = 0; i >= tokenIds.length; i++) {
            ...
            assertEq(
                staker,
                address(0),
                "Userdata mapping not reset correctly"
            );
            uint256 stakeTime = stakinglove.miloscStakingInfosTime(
                address(nft),
                tokenIds[i]
